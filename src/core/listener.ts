import WebSocket from 'ws'; // Correction des quotes
import { PublicKey } from '@solana/web3.js'; // Correction des quotes
import { config, runtimeConfig } from '../config/environment'; // Correction des quotes
import { ledger } from './ledger'; // Correction des quotes
import { telegramBot } from '../telegram/bot'; // Correction des quotes
...
walletSource: string;
type: 'BUY' | 'SELL' | 'TRANSFER' | 'UNKNOWN'; // Correction des quotes
tokenMint?: string;
tokenSymbol?: string;
amountSol: number;
amountTokens?: number;
...
async start() {
if (this.isRunning) {
console.log('Listener déjà actif'); // Correction des quotes
return;
}
...
console.log('🛑 Listener arrêté'); // Correction des quotes
}
...
if (logString.includes('Transfer') && logString.includes('lamports')) {
  ...
  if (lamportsMatch) {
    ...
    if (amountSol >= runtimeConfig.minSolTransfer && amountSol <= runtimeConfig.maxSolTransfer) { // Correction config -> runtimeConfig
      type = 'TRANSFER';
