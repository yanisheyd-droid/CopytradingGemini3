...
private _isRunning: boolean = false; // Renommé pour éviter TS2300

constructor() {
this.connection = new Connection(config.rpcHttps, 'confirmed'); // Correction des quotes
}

start() {
if (this._isRunning) {
console.log('Discovery Wallet déjà actif'); // Correction des quotes
return;
}
...
console.log(`   Min: ${runtimeConfig.minSolTransfer} SOL`); // Correction config -> runtimeConfig
console.log(`   Max: ${runtimeConfig.maxSolTransfer} SOL`); // Correction config -> runtimeConfig
}

stop() {
this._isRunning = false;
console.log('🛑 Discovery Wallet arrêté'); // Correction des quotes
}

// Cette méthode est appelée par le listener quand un transfer est détecté
...
if (!this._isRunning) return; // Correction de la propriété

// Vérifier si c'est dans la fourchette
if (amount < runtimeConfig.minSolTransfer || amount > runtimeConfig.maxSolTransfer) { // Correction config -> runtimeConfig
  return;
}
...
isRunning(): boolean {
return this._isRunning; // Utilisation de la propriété renommée
}
